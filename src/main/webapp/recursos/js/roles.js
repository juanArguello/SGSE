$(document).ready(function () {
    ...
    // Resetea los datos de la lista de permisos al cerrar el modal
    $('#listaPermisosModal').on('hidden.bs.modal', function (event) {
        $("#tituloModal").empty();
    ...
    $("#tabla_roles tbody").on("click", ".btnVerPermiso", function(){
        ...
        datosRol = $('#tabla_roles').DataTable().row(fila).data();
        // Cuando la tabla de datos es responsive
        if(datosRol === undefined) {
            var selected_row = $(this).parents('tr');
            if (selected_row.hasClass('child')) {
                selected_row = selected_row.prev();
            }
            let  rowData = $('#tabla_roles').DataTable().row(selected_row).data();
            id = parseInt(rowData.id); //capturo el ID
            nombre = rowData.nombre;
            $("#tituloModal").text("Lista de permisos de "+nombre);
            $.ajax({
                url: urlEndPoint+"/roles/"+id,
                type: 'GET',
            ...
            $("#tituloModal").text("Lista de permisos de "+datosRol.nombre);
            let  filasPermisos = '';
            $.each(datosRol.permisosList, function (i, item) {
                filasPermisos += '<tr>';
                filasPermisos += '<td>' + item.id + '</td>';
                filasPermisos += '<td>' + item.nombre + '</td>';
                filasPermisos += '<td>' + item.descripcion + '</td>';
    ...
    $("#tabla_roles tbody").on("click", ".btnEditar", function(){
        ...
        opcion = 2; //editar
        location = urlRoot+"/administracion/roles/edit/"+id;
    });
